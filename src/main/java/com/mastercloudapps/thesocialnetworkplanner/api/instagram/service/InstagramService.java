...
public class InstagramService {
    ...
    public InstagramResponse scheduleInstagram(String caption, MultipartFile multipartFile, Date publishDateStore) throws InstagramException {
        String url = this.resourceService.createImage(multipartFile);
        Instagram instagramPost = this.instagramRepository.save(Instagram.builder().creationDate(new Date()).scheduledDate(publishDateStore).text(caption).username(this.getAccount()).build());
        Resource resource = this.resourceService.saveResource(Resource.builder().url(url).creationDate(new Date()).instagram(instagramPost).build());
    ...
    public List<Schedulable> getUnpublishedPosts() {
        return this.instagramRepository.findByInstagramIdIsNull();
    }
}
